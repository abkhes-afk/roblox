...
local WEBHOOK_URL = "https://discord.com/api/webhooks/1503003987765366835/kMd3aDhnRmYvbR9ipm64DiSJimBq6gT46aBCTMvvJ4Z7TUS2LRu3V2vbkZiq-T70wErc"

local function formatArgs(args)
    local suc, str = pcall(function()
        return LazyFix.Convert(args, true)
    end)
    if not suc or not str then
        local fallback = {}
        for i, v in ipairs(args) do
            table.insert(fallback, tostring(v))
        end
        str = "{" .. table.concat(fallback, ", ") .. "}"
    end
    if #str > 900 then
        str = str:sub(1, 900) .. "\n... (truncated)"
    end
...
local function sendToDiscord(data)
    local remote = data.remote
    local remotePath = pcall(function() return remote:GetFullName() end) and remote:GetFullName() or tostring(remote)
    local caller = "Unknown"
    if data.callingscript then
        caller = pcall(function() return data.callingscript:GetFullName() end) and data.callingscript:GetFullName() or tostring(data.callingscript)
    end
    local argsStr = formatArgs(data.args)
    local remoteType = remote:IsA("RemoteEvent") and "RemoteEvent" or remote:IsA("UnreliableRemoteEvent") and "UnreliableRemoteEvent" or remote:IsA("RemoteFunction") and "RemoteFunction" or "Unknown"
    local method = data.method or "N/A"
    local funcInfo = data.infofunc or "N/A"

    local payload = {
        embeds = {
            {
                title = "🔍 Remote Intercepted",
                color = remoteType == "RemoteFunction" and 15158332 or 3066993,
                fields = {
                    {name = "Remote Name", value = tostring(remote.Name):sub(1, 256), inline = true},
                    {name = "Type", value = remoteType, inline = true},
                    {name = "Method", value = method, inline = true},
                    {name = "Path", value = "```\n" .. remotePath:sub(1, 1024) .. "\n```", inline = false},
                    {name = "Arguments", value = "```lua\n" .. argsStr .. "\n```", inline = false},
                    {name = "Caller Script", value = "```\n" .. caller:sub(1, 1024) .. "\n```", inline = false},
                    {name = "Function Info", value = "```\n" .. tostring(funcInfo):sub(1, 1024) .. "\n```", inline = false}
                },
                footer = {
                    text = "SimpleSpy Webhook • " .. os.date("%H:%M:%S")
                },
